...
> **设计原则**：v1.0 是「事后擦除」→ v2.0 是「事前隔离」→ v2.1 是「事前隔离 + 事后全量反向审计」→ v2.2.1 是「稳健性硬化」→ v2.3.0 是「实战修正」→ v2.3.1 是「去重修正」→ v2.4.0 是「盲区填补：cache+tmp+subagent+python_history 四盲区关闭 + 进程审计降级」→ v2.4.1 是「终端快照泄漏修复：hermes-snap-*.sh 纳入强制覆写」→ v2.5.0 是「cache 盲区细化：web/screenshots/videos 三用户数据目录通过符号链接重定向至沙箱，事前隔离替代事后擦除，崩溃安全」→ v2.5.2 是「Skill 审计降噪：4.4 find 命令排除 .curator_state/.bundled_manifest/.usage.json 元数据文件」→ v2.5.3 是「R4 对照真实架构深度审计修复：4.6b 清洗改时间戳窗口（原正则 0 命中静默失效）、4.1a delegation live transcripts 专项审计、向量索引哨兵（防无痕内容入持久向量库）、Phase 4 恢复块拆分（bash -c export 不持久）、cache 重定向扩展 7 目录 + 悬空链接自愈、4.1 排除列表补充、能力矩阵补 gbrain/agy/execute_code、4.6c snap 二次清除、profile-safe 路径」→ v2.5.4 是「R5 复检补修：profile-safe 完整落地到 4 处运行路径（P1-9/4.3/4.4/4.6b）、Phase 4 恢复块补恢复 INCOGNITO_HERMES_HOME、4.1b 排除哨兵点文件、文档引用与过时描述修正」→ v2.5.5 是「R6 实测验证修复：真实无痕会话暴露 agent.log 双盲区——firecrawl 等 provider 层 `Firecrawl search:` 日志格式与 `conversation turn` 用户消息明文，4.6b 扩展覆盖 7 种 provider 格式 + msg 行（repr 双引号兼容）+ URL 行；哨兵语义重构——精细跳过（只跳过 active_sessions 源）+ 4.9 删 session 隐式释放（采纳用户思路），根治 Phase 5 漏删导致的向量任务阻塞」→ v2.5.6 是「R7 采纳两项：① interrupted_turns.json 纳入 Phase 4 审计（4.1c，防崩溃残留 prompt 明文）；② Logging RedactingFilter 通过用户插件落地（~/.hermes/plugins/incognito-log-filter/，哨兵门控 + 内存脱敏，事前拦截替代事后擦除，与 4.6b 双层防御）」→ v2.5.7 是「R8 依赖显式化：Phase 1 新增步骤 3.6 检查 incognito-log-filter 插件是否安装/启用——防静默失效（用户以为有双层防御，实际 filter 未加载只有 4.6b 兜底）；软警告不阻塞」→ v2.5.8 是「R9 子代理 E2E 现场修复：① async_delegations 表纳入 4.1a 审计（delegate_task 任务书/结果持久化进 state.db，4.9 不清理——按 sid 精确删除）；② unset 时机修正（原 4.6 处 unset 会让 4.6b/4.7b/4.8 变量为空静默失效——移至 4.8 之后）」→ v2.5.9 是「R10 发布前全面复检修复：4.6b 裸单引号破坏 bash 单引号包裹（`python3 -c '...'` 内嵌 `(["'])` 正则与注释 `'xxx'` 导致 bash 字符串提前闭合——真实 terminal 执行静默失败，P0）；改为 chr 拼接 + 注释去引号，30/30 bash 块 bash -n 全过」。不信任 Phase 2 的隔离完美无缺——发现痕迹 → 报告 → 清除 → 二次验证。
> ⚡ **三条铁律（每次行动前自检）**：
